...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    YearActivityMarkView *yearActivityMarkView = [[YearActivityMarkView alloc] initWithFrame:self.activityView.bounds];
    yearActivityMarkView.date = date;
    yearActivityMarkView.dataValueType = dataValueType;
    [self.activityView addSubview:yearActivityMarkView];
...
-(void)drawResult{
    ...
    if(dataValueType == DATA_VALUE_TYPE_STEP){
        ...
        NSDate *firstDayOfYear = [calendar dateFromComponents:dateComps];
        
        DatabaseHelper *dbHelper = [[DatabaseHelper alloc] init];
        
        //１ヶ月分のランニングの詳細を取得
        NSMutableArray *yearRunDetailArray = [dbHelper selectYearRunDetail:firstDayOfYear];
        
        int sumRunStepValue = 0;
        int sumRunSeconds = 0;
        
        //歩数と時間の合計値を計算
        for(NSMutableDictionary *runDetail in yearRunDetailArray) {
