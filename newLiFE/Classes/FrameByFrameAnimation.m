                    ...
                    :(NSInteger)animationRepeatNum
{
    ...
    //最後の画像が消えないようにする
    _animationImageView.image = [self getUIImageFromResources:[self animationImageName:animationImagePrefix :animationImageNum - 1]
...
- (NSString *)animationImageName:(NSString *)animationImageSuffix
                                :(int)animationFrame
{
    NSString *imageName = [NSString stringWithFormat:@"%@%02d", animationImageSuffix, animationFrame];
    return imageName;
}
...
{
    NSString *imageName = [NSString stringWithFormat:@"%@.%@", fileName, ext];
